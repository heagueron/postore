...
<div>
    ...
    <input  data-required="required" autocomplete="off" id="companyNameElement"
            ...
            value="{{ !is_null( old('company_name'))? old('company_name') : $user->companies->first()->name }}"               
    >
    {{-- Hidden input to hold company id --}}
    <input  type="hidden" id="companyIdElement" name="company_id" 
            value="{{ !is_null( old('company_id'))? old('company_id') : $user->companies->first()->id }}">
...
<div>
    ...
    <input  data-required="required" autocomplete="off" id="companyEmailElement"
            ...
            value="{{ !is_null( old('company_email'))? old('company_email') : $user->companies->first()->email }}"               
    >
    <span class="rp-group__info">
