...
class DailyTwitter extends Command
{
    use PublishRemjob;
     ...
     * @var string
     */
    protected $signature = 'postore:dailyTwitter';
     ...
     * @var string
     */
    protected $description = 'Share remjobs on twitter 1 and 3 days after activation';
    ...
    public function handle()
    {
        if( \App\Remjob::where('active', 1)->exists() ){
            ...
            foreach ( $remjobs as $remjob ) {
                ...
                if( ( $remjob->twitterPosts()->count() == 3 and now()->diffInDays($remjob->created_at) > 1 ) 
                    or 
                    ($remjob->twitterPosts()->count() == 4 and now()->diffInDays($remjob->created_at) > 3) ){
